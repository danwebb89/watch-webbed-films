document.addEventListener('DOMContentLoaded', async () => {
  ...
  function renderFilms(films) {
    ...
    grid.innerHTML = films.map(film => {
      ...
      return `
      <a class="portfolio-card${locked ? ' portfolio-card-locked' : ''}" data-slug="${esc(film.slug)}" data-title="${esc(film.title)}" data-locked="${locked}" href="/watch/${encodeURIComponent(film.slug)}">
        <div class="card-thumb">
          <img src="${esc(film.thumbnail)}" alt="${esc(film.title)}" loading="lazy" onerror="this.style.display='none'">
          ${lockIcon}
        </div>
        <div class="card-overlay">
          <div class="card-overlay-title">${esc(film.title)}</div>
          <div class="card-overlay-meta">${esc(film.category)} — ${esc(String(film.year || ''))}</div>
          ${cta}
        </div>
      </a>`;
    }).join('');
